...
@sleep_and_retry
@limits(calls=150, period = timedelta(seconds=60).total_seconds())
...
def get_film_reviews(root_url, urls, max_reviews_per_film=None):
    ...
    for i, url in enumerate(urls):
        ...
        resp= requests.head(film_url)
        dic_index[film_url] = 1
        if resp.status_code != 200:
            print(f"Film with url {film_url} not found")
            dic_index[film_url] = 0
            continue
        else:
